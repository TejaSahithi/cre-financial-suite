...
describe("LeaseReviewTabTable contract", () => {
  ...
  it("separates filled rows from missing/source-only rows and cleans source previews", () => {
    ...
    expect(source).toContain("Mandatory");
    expect(source).toContain("isMandatoryRow");
  ...
  it("defaults typeFilter to standard fields when available", () => {
    expect(source).toContain('rows.some((row) => row.rowType === "standard") ? "standard" : "all"');
  });
});
