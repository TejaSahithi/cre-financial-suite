...
import { TrendingUp, TrendingDown, Building2, DollarSign, ChevronRight } from "lucide-react";
import { Link } from "react-router-dom";
import { createPageUrl } from "@/utils";

const COLORS = ["#3b82f6", "#10b981", "#f59e0b", "#8b5cf6", "#ef4444", "#06b6d4", "#ec4899", "#1e293b"];

export default function InsightsTab({ properties, leases, expenses, camCalcs }) {
  ...
  const PropertyRow = ({ p, i }) => (
    <Link to={`${createPageUrl("PropertyDetail")}?id=${p.id}`} className="flex items-center gap-3 py-2 border-b border-slate-50 last:border-0 hover:bg-slate-50 rounded group">
      ...
      <div className="text-right">
        <p className={`text-sm font-bold tabular-nums ${p.noi >= 0 ? 'text-emerald-700' : 'text-red-600'}`}>{fmt(p.noi)}</p>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card className="border-0 shadow-sm">
          ...
          <CardContent className="px-4 pb-3 pt-1">
            <div className="space-y-2">{topByNOI.slice(0, 5).map((p, i) => <PropertyRow key={p.id} p={p} i={i} />)}</div>
          </CardContent>
        </Card>
        ...
        <Card className="border-0 shadow-sm">
          ...
          <CardContent className="px-4 pb-3 pt-1">
            <div className="space-y-2">{bottomByNOI.slice(0, 5).map((p, i) => <PropertyRow key={p.id} p={p} i={i} />)}</div>
          </CardContent>
        </Card>
