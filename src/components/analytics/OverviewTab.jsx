...
import { BarChart3, FileText, DollarSign, Calculator, TrendingUp, Building2 } from "lucide-react";
...
export default function OverviewTab({ properties, leases, expenses }) {
  ...
  const chartData = MONTHS.map((month, i) => ({
    month, revenue: monthlyRent, expenses: expByMonth[i + 1] || 0, noi: monthlyRent - (expByMonth[i + 1] || 0),
  }));
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
            {reportTypes.map((r, i) => (
              <ModuleLink key={i} page={r.page} className="flex items-center gap-3 p-4 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors group">
