...
export default function FinancialSummaryStrip({ revenue, expenses, noi, budgeted, camRecovery, occupancy, rentPerSF, noiMargin }) {
  const metrics = [
    ...
    { label: "NOI", value: fmt(noi), tone: noi >= 0 ? "border-blue-200/80 bg-blue-50/80 text-blue-800" : "border-rose-200/80 bg-rose-50/80 text-rose-700" },
    { label: "Budgeted", value: fmt(budgeted), tone: "border-blue-200/80 bg-blue-50/80 text-blue-800" },
    { label: "CAM Recovery", value: fmt(camRecovery), tone: "border-blue-200/80 bg-blue-50/80 text-blue-800" },
  ...
  return (
    <div className="grid w-full grid-cols-1 gap-2 rounded-[10px] border border-[var(--border-cre)] bg-[var(--pf-surface-card)] p-2 shadow-[var(--card-shadow)] backdrop-blur-xl min-[420px]:grid-cols-2 md:grid-cols-4 2xl:grid-cols-8">
      {metrics.map((m) => (
        <div key={m.label} className={`min-h-[64px] rounded-lg border px-3 py-2 text-center transition-[border,background,box-shadow,transform] duration-200 hover:-translate-y-[1px] hover:shadow-[var(--shadow-soft)] ${m.tone}`}>
          <p className="mb-1 text-[11px] font-bold uppercase leading-none tracking-wide text-[var(--muted)]">{m.label}</p>
