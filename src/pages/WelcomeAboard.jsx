...
// Rich module registry with detailed summaries
const MODULE_RICH = {
  dashboard: {
    icon: BarChart2,
  ...
  analytics_reports: {
    ...
    desc: "Unlock powerful reporting — NOI analysis, rent projection, market benchmarks, and exportable dashboards for investor-ready presentations.",
  ...
  vendors: {
    ...
    desc: "Manage your service provider ecosystem — contract terms, insurance tracking, invoice history, and performance ratings.",
  ...
  reconciliation: {
    ...
    desc: "Reconcile bank statements, rent rolls, and payable ledgers against your operational data. Catch discrepancies before they become problems.",
  ...
  workflows: {
    ...
    desc: "Automate approval workflows, maintenance tickets, notifications, and task assignments. Keep your team moving without manual follow-ups.",
  ...
  documents: {
    ...
    desc: "Secure storage for leases, contracts, insurance certs, and inspection reports. Full-text search and permission-based access.",
    color: "from-slate-500 to-slate-600",
    badge: "Core"
  },
};

// SuperAdmin modules - shows platform management capabilities
const SUPER_ADMIN_FEATURES = [
  {
    icon: Shield,
    label: "Platform Administration",
    desc: "Full visibility across all organizations. Approve/reject client accounts, monitor platform health, and manage access requests from a unified console.",
  ...
  },
  {
    ...
    desc: "Manage every user across all organizations. Assign roles, revoke access, review invitations, and control permissions at the org or user level.",
  ...
  },
  {
    ...
    desc: "Aggregate and compare performance metrics across all client organizations. Generate platform-level reports for internal review.",
  ...
  },
  {
    ...
    desc: "Configure system defaults, module availability flags, email templates, and security settings that govern the entire platform.",
  ...
  },
  {
    ...
    desc: "All modules — Portfolio, Leases, CAM, Budgets, Analytics, Vendors, and more — are available to you across every client organization.",
    color: "from-emerald-600 to-teal-600",
    badge: "All Access"
  },
];
...
export default function WelcomeAboard() {
  ...
  const getFeatures = () => {
    // SuperAdmin sees platform admin capabilities
    if (isSuperAdmin) return SUPER_ADMIN_FEATURES;

    // OrgAdmin sees ALL modules
    if (isOrgAdmin) {
      return Object.entries(MODULE_RICH).map(([, v]) => v);
    }

    // Regular users see their assigned modules only
    const assigned = enabledModules?.length > 0
      ? enabledModules
      : ['dashboard', 'portfolio', 'analytics_reports'];

    return assigned
      .map(key => MODULE_RICH[key] || {
        icon: Sparkles,
  ...
  const getRoleLabel = () => {
    ...
    return { text: "Team Member", color: "bg-blue-100 text-blue-700 border border-blue-200" };
  };
      ...
      <div className="max-w-5xl w-full relative z-10">
        ...
        <div className="text-center mb-14 animate-in fade-in slide-in-from-bottom-6 duration-700">
          ...
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            {!isSuperAdmin && (
              <span className="text-slate-500 text-sm font-medium">{orgName}</span>
            )}
          </div>
          <p className="text-slate-500 text-base max-w-2xl mx-auto leading-relaxed">
            {isSuperAdmin
              ...
              : isOrgAdmin
              ? `Your entire workspace for ${orgName} is ready. As Organization Admin, you have access to all modules in the suite — here's what's at your fingertips.`
              : `Your workspace for ${orgName} is ready. Explore your assigned modules below and start making data-driven decisions today.`
            }
          </p>
        </div>
        ...
        {(isOrgAdmin || isSuperAdmin) && (
          <div className="grid grid-cols-3 gap-4 mb-14 animate-in fade-in duration-700 delay-500 fill-mode-both">
            {[
              ...
              <div key={i} className="bg-white rounded-2xl border border-slate-200 px-6 py-5 text-center shadow-sm">
