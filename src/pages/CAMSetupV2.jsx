/**
 * CAMSetupV2 — Phase 4B: complete, guided 7-step CAM setup workflow.
 *
 * An authorized property manager can complete full CAM setup without direct
 * SQL or manual RPC calls. Every write routes through cam-setup-actions-v2.
 *
 * Workflow:
 *   Step 1 — Property / Period:  select property, create calendar, create period
 *   Step 2 — Pools:              create pool, assign categories, configure gross-up
 *   Step 3 — Participants:       add/remove lease participants per pool
 *   Step 4 — Policies:           review materialized policies, resolve UNKNOWN values
 *   Step 5 — Expenses:           assign published expenses to pools
 *   Step 6 — Estimates / Adjustments: monthly schedules + prior adjustments
 *   Step 7 — Readiness:          run readiness check, see blocking items, launch run
 */
...
function SeverityBadge({ severity }) {
  const tone =
    severity === "blocking"       ? "bg-red-100 text-red-700"
    : severity === "warning"      ? "bg-amber-100 text-amber-800"
    : severity === "review_required" ? "bg-blue-100 text-blue-700"
    : "bg-slate-100 text-slate-600";
  return <Badge className={`text-[10px] uppercase ${tone}`}>{severity}</Badge>;
}
...
// Call cam-setup-actions-v2 edge function.
...
const STEPS = [
  ...
  { id: 6, label: "Estimates",         icon: DollarSign },
  { id: 7, label: "Readiness",         icon: CheckCircle2 },
];

// ---- Main component ---------------------------------------------------------

export default function CAMSetupV2() {
  ...
  const [policyDialog, setPolicyDialog] = useState(null);  // exception row
  const [expenseDialog, setExpenseDialog] = useState(null); // expense input row
  const [estimateDialog, setEstimateDialog] = useState(false);
  const [priorAdjDialog, setPriorAdjDialog] = useState(null); // { leaseId, periodId, adjType }
  ...
  const { data: pools = [], refetch: refetchPools } = useQuery({
    queryKey: ["recovery_pools", periodId],
    enabled: !!periodId,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("recovery_pools")
  ...
  const {
    ...
    enabled: !!propertyId && !!periodId && step === 7,
    queryFn: async () => {
    ...
    return (
      <div className="space-y-6">
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Gross-Up Target</TableHead>
                  ...
                  <TableBody>
                    {pools.map((pool) => (
                      <TableRow key={pool.id}>
                        ...
                        <TableCell>
                          {pool.default_gross_up_target_pct != null ? `${pool.default_gross_up_target_pct}%` : "—"}
                        </TableCell>
                        <TableCell>
                          <div className="flex gap-1">
                            ...
                            <Button size="sm" variant="outline" id={`btn-grossup-${pool.id}`}
                              onClick={() => { setSelectedPoolId(pool.id); setGrossUpDialog(true); }}>Gross-Up</Button>
                          </div>
                        </TableCell>
                      </TableRow>
                    ))}
        ...
        <Dialog open={grossUpDialog} onOpenChange={setGrossUpDialog}>
          <DialogContent>
            <DialogHeader><DialogTitle>Configure Gross-Up Target</DialogTitle></DialogHeader>
            <div className="space-y-3">
              <div>
                <Label>Target Occupancy % (leave blank to disable)</Label>
            ...
            <DialogFooter>
              ...
              <Button id="btn-save-grossup" onClick={async () => {
                ...
                await doAction("configure_pool_grossup", { pool_id: selectedPoolId, default_gross_up_target_pct: targetPct },
                  [["recovery_pools", periodId]], "Gross-up configured");
  ...
  function Step4() {
    ...
    const unknownPolicies = policies.filter((p) => !p.status || p.status === "draft");

    return (
      <div className="space-y-6">
        <Card>
          <CardHeader><CardTitle className="text-base">Materialized Lease Policies</CardTitle></CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow>
                    ...
                    <TableCell>
                      {pol.status !== "approved" && (
                        <Button size="sm" variant="outline" id={`btn-resolve-policy-${pol.id}`}
                          onClick={() => setPolicyDialog(pol)}>Resolve</Button>
                      )}
                    </TableCell>
                  </TableRow>
                ))}
  ...
  function Step5() {
    ...
    return (
      <div className="space-y-4">
        <Card>
          <CardHeader><CardTitle className="text-base">Published Expense Inputs</CardTitle></CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableRow key={exp.id}>
                    ...
                    <TableCell>
                      <Button size="sm" variant="outline" id={`btn-assign-expense-${exp.id}`}
                        onClick={() => setExpenseDialog(exp)}>Assign</Button>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
            ...
            {!readinessLoading && readiness && (
              <div className="space-y-4">
                ...
                {isReady && (
                  ...
                  >
                    <Button className="mt-2" id="btn-launch-cam-run">Run CAM Calculation →</Button>
                  </Link>
                )}
              </div>
            )}
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="CAM Setup"
        subtitle="Complete guided setup to prepare a CAM calculation run"
      />
