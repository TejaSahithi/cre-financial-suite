...
export default function CriticalDates() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async ({ id, leaseId }) => deleteCriticalDate({ id, leaseId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["lease-critical-dates"] });
                      ...
                      <div className="flex gap-1">
                        ...
                        {row.status !== "completed" && !row.preview_only && (
                          <Button
                            ...
                            onClick={() => completeMutation.mutate({ id: row.id, leaseId: row.lease_id, by: row.owner_name || row.owner_email })}
