...
export const options = {
  scenarios: {
    steady: { executor: "constant-vus", vus: 25, duration: "1m" },
  },
  thresholds: {
...
export default function () {
  const res = http.post(`${BASE_URL}/document-field-review-action`, JSON.stringify({ uploadedFileId: __ENV.UPLOADED_FILE_ID || "load-file", runId: __ENV.RUN_ID || "load-run", generationId: __ENV.GENERATION_ID || "load-generation", canonicalFieldKey: "tenant_name", action: "accept" }), { headers, tags: { organization: ORGANIZATION_ID, stale: "false" } });
