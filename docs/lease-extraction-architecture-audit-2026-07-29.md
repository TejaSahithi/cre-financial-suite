...
- Lease expense/CAM rules: generated by the canonical lease workflow as `workflow_output.expense_rules`, then persisted through `leaseExpenseRuleService.persistExpenseRulesFromWorkflow()` / `save-lease-expense-rule-set`. Existing approved abstracts can be explicitly backfilled from the Lease Expense Rules page.
