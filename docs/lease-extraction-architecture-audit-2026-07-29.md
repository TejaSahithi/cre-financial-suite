...
- Lease expense/CAM obligations: extracted by the same strict whole-document LLM call that produces Lease Review fields. The evidence validator converts the LLM's `expenseRuleCandidates` into `workflow_output.expense_rules` without a TypeScript semantic re-extraction pass.
- Approval-only publication: candidates remain inside the versioned review/workflow payload before approval. `approve-lease-workflow` and the explicit approved-lease sync endpoint call one shared server publisher, which materializes the exact candidates into `lease_expense_rule_sets`, `lease_expense_rules`, values, and clauses. The publisher never invokes an LLM and never regenerates semantic rules from keywords.
- Existing abstracts created before the canonical expense-obligation contract must be re-extracted. The approval publisher returns `approved_lease_reextraction_required` instead of silently falling back to the retired TypeScript blueprint projection.
