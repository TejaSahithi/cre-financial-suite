...
let SERVICE_KEY = process.env.VITE_SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY;
let STRIPE_SECRET = process.env.STRIPE_SECRET_KEY;
...
if (!SUPABASE_URL) {
  try {
    ...
    const getVal = (key) => {
        const m = envStr.match(new RegExp(key + '=(.*)'));
        return m ? m[1].trim() : undefined;
    };
    if (!SUPABASE_URL) SUPABASE_URL = getVal('VITE_SUPABASE_URL') || getVal('SUPABASE_URL');
    if (!SUPABASE_ANON_KEY) SUPABASE_ANON_KEY = getVal('VITE_SUPABASE_ANON_KEY') || getVal('SUPABASE_ANON_KEY');
    if (!SERVICE_KEY) SERVICE_KEY = getVal('VITE_SUPABASE_SERVICE_ROLE_KEY') || getVal('SUPABASE_SERVICE_ROLE_KEY');
    if (!STRIPE_SECRET) STRIPE_SECRET = getVal('STRIPE_SECRET_KEY');
    if (!STRIPE_WEBHOOK_SECRET) STRIPE_WEBHOOK_SECRET = getVal('STRIPE_WEBHOOK_SECRET');
  } catch(e) {}
}
...
async function run() {
  ...
  if (!STRIPE_SECRET || !STRIPE_WEBHOOK_SECRET) {
      console.log("Skipping webhook tests: missing STRIPE_SECRET or STRIPE_WEBHOOK_SECRET in .env");
  } else {
      let stripe;
      try {
        stripe = require('stripe')(STRIPE_SECRET);
      } catch (e) {
        console.log("Missing stripe npm package, skipping webhook sign generation");
      }
