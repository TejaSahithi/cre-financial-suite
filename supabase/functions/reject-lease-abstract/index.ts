...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    await logger?.event("abstract_reject", "started", {
    ...
    if (error) {
      await logger?.event("abstract_reject", "failed", { lease_id: payload.leaseId, reason: error.message });
      throw new Error(error.message || "reject_lease_abstract failed");
    }

    await logger?.event("abstract_reject", "succeeded", { lease_id: payload.leaseId, ui: uiContext });
