...
import { DOMAIN_THRESHOLDS, LLM_CALL_DOMAINS } from "../_shared/extraction/section-router.ts";
...
Deno.test("byte-compat: every domain's criticalFields deep-equals the legacy CRITICAL_FIELDS_BY_DOMAIN entry", () => {
  for (const id of LEGACY_DOMAIN_IDS) {
    ...
    assertEquals([...definition.criticalFields], LEGACY_CRITICAL_FIELDS[id], `criticalFields mismatch for ${id}`);
  }
});
...
Deno.test("byte-compat: every domain's promptConcepts is character-identical to the legacy DOMAIN_CONCEPTS entry", () => {
  for (const id of LEGACY_DOMAIN_IDS) {
    const definition = getDomainDefinition(id as any);
    assertEquals(definition.promptConcepts, LEGACY_PROMPT_CONCEPTS[id], `promptConcepts mismatch for ${id}`);
    ...
    assertEquals(definition.promptConcepts.length, LEGACY_PROMPT_CONCEPTS[id].length);
  }
});

Deno.test("byte-compat: DOMAIN_THRESHOLDS deep-equals the legacy routing thresholds", () => {
  for (const id of LEGACY_DOMAIN_IDS) {
    assertEquals(DOMAIN_THRESHOLDS[id], LEGACY_ROUTING_THRESHOLDS[id], `routingThreshold mismatch for ${id}`);
  }
});

Deno.test("byte-compat: ENRICH_EVIDENCE_DOMAIN_STAGES deep-equals the legacy array", () => {
  assertEquals(ENRICH_EVIDENCE_DOMAIN_STAGES, [...LEGACY_ENRICH_EVIDENCE_DOMAIN_STAGES]);
});

Deno.test("byte-compat: ENRICH_STAGE_SEQUENCE (all 10 stages) deep-equals the legacy array", () => {
  assertEquals([...ENRICH_STAGE_SEQUENCE], [...LEGACY_ENRICH_STAGE_SEQUENCE]);
});
...
Deno.test("byte-compat: every registry domain's boundedEnrichStageName maps back to the legacy STAGE_TO_LLM_CALL_DOMAIN entry", () => {
  for (const [stage, domainId] of Object.entries(LEGACY_STAGE_TO_LLM_CALL_DOMAIN)) {
    const definition = DOMAIN_REGISTRY.find((d) => d.boundedEnrichStageName === stage);
    assertEquals(definition?.id, domainId, `stage ${stage} should map to domain ${domainId}`);
  }
});
