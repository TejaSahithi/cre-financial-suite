 ...
 * Feature: backend-driven-pipeline, Task 6.5
 *
 * **Validates: Requirements 4.3, 4.5**
 *
 * Property 14: Referential Integrity Enforcement
 * For any store-data call, all inserted rows must have the correct org_id.
 */
...
function validRowsFor(moduleType: string, count: number): Record<string, unknown>[] {
  return Array.from({ length: count }, (_, i) => {
    switch (moduleType) {
      case "leases":
        return { tenant_name: `Tenant ${i + 1}`, start_date: "2024-01-01", end_date: "2025-12-31", monthly_rent: 1000 };
      case "expenses":
        return { category: "Utilities", amount: 300 + i * 10, date: "2024-06-01" };
      case "properties":
        return { name: `Building ${i + 1}` };
      case "revenue":
        return { revenue_type: "cam_recovery", amount: 500 + i * 50 };
      default:
        return {};
    }
  });
...
Deno.test({
  name: "Property 14: Referential Integrity Enforcement - all inserted rows have correct org_id",
      ...
      fc.asyncProperty(
        ...
        async (moduleType, rowCount) => {
          ...
          try {
            ...
            // Fetch all rows inserted for this org
            const { data: dbRows } = await adminClient
              .from(tableName)
              .select("id, org_id")
              .eq("org_id", org.id);

            assertExists(dbRows, "Must be able to query inserted rows");

            // Property: every row must have the correct org_id
            for (const row of dbRows ?? []) {
              assertEquals(
                row.org_id,
                org.id,
                `Row ${row.id} must have org_id=${org.id}, got ${row.org_id}`,
              );
            }

            // Property: no rows from other orgs should be present
            const { data: otherOrgRows } = await adminClient
              .from(tableName)
              .select("id, org_id")
              .neq("org_id", org.id)
              .in("id", (dbRows ?? []).map((r: any) => r.id));

            assertEquals(
              (otherOrgRows ?? []).length,
              0,
              "No inserted rows should belong to a different org",
            );
