...
import { checkStageInputAgainstLimits, getEnrichStageLimits } from "../_shared/extraction/enrich-stage-limits.ts";
import {
...
// ---------------------------------------------------------------------------
function makeMockSupabase({ rows, rpcResults = {}, activeGenerationOverride = null }: {
  ...
  function from(table: string) {
    const builder: any = {
      ...
      update(patch: any) { builder._isUpdate = true; builder._patch = patch; return builder; },
...
// 1. Exact stage order
// ===========================================================================
Deno.test("enrich-bounded-stages: exact stage order -- 4 workflow stages, 5 evidence domains, 1 final assembly, in that order", () => {
  assertEquals(ENRICH_STAGE_SEQUENCE, [
    "enrich_clauses", "enrich_fields", "enrich_items", "enrich_derivation",
    "enrich_evidence_core_terms", "enrich_evidence_rent_and_charges", "enrich_evidence_expenses_and_cam",
  ...
  assertEquals(nextEnrichBoundedStage("enrich_derivation"), "enrich_evidence_core_terms");
  assertEquals(nextEnrichBoundedStage("enrich_evidence_legal_rights_and_dates"), "enrich_truth_assembly");
...
Deno.test("enrich-bounded-stages: a successful non-final stage enqueues EXACTLY the next stage, never more", async () => {
  const supabaseAdmin = makeMockSupabase({
    rows: { uploaded_files: { active_generation_id: GEN_ID }, pipeline_jobs: { id: JOB_ID, status: "running" } },
    rpcResults: { enqueue_pipeline_job: { data: { created: true, job_id: "job-2" }, error: null } },
  });
  const job = { id: JOB_ID, generation_id: GEN_ID, metadata: {} };
  const result = await completeBoundedEnrichStage({
    supabaseAdmin, job, fileId: FILE_ID, orgId: ORG_ID, stage: "enrich_clauses", outcome: { ok: true },
  });
  assertEquals(result.nextStage, "enrich_fields");
  const enqueueCalls = supabaseAdmin.__rpcCalls.filter((c: any) => c.name === "enqueue_pipeline_job");
...
Deno.test("enrich-bounded-stages: isStageAlreadyCompleted reuses a matching completed entry, rejects mismatched generation/version", () => {
  const normalizedOutput = mergeBoundedStageResult({
    normalizedOutput: {}, stage: "enrich_clauses", generationId: GEN_ID, status: "completed", data: { clauses: [] },
  });
  assertEquals(isStageAlreadyCompleted({ normalizedOutput, stage: "enrich_clauses", generationId: GEN_ID }).reusable, true);
  assertEquals(isStageAlreadyCompleted({ normalizedOutput, stage: "enrich_clauses", generationId: "other-gen" }).reusable, false);
  assertEquals(isStageAlreadyCompleted({ normalizedOutput, stage: "enrich_fields", generationId: GEN_ID }).reusable, false);
  ...
  const staleVersionOutput = {
    metadata: { extractionDebug: { bounded_stage_results: { enrich_clauses: { status: "completed", generation_id: GEN_ID, stage_version: "v0-stale", data: {} } } } },
  };
  assertEquals(isStageAlreadyCompleted({ normalizedOutput: staleVersionOutput, stage: "enrich_clauses", generationId: GEN_ID }).reusable, false);
});
...
Deno.test("enrich-bounded-stages: handleBoundedEnrichStage returns reused_from_cache=true and does NOT recompute for an already-completed stage", async () => {
  const priorNormalizedOutput = mergeBoundedStageResult({
    normalizedOutput: { rows: [{ tenant_name: "Acme" }], method: "llm_only", warnings: [], validationErrors: [], metadata: {} },
    stage: "enrich_clauses", generationId: GEN_ID, status: "completed", data: { clauses: [] },
  });
  const supabaseAdmin = makeMockSupabase({
    rows: { uploaded_files: { id: FILE_ID, org_id: ORG_ID, active_generation_id: GEN_ID, module_type: "lease", docling_raw: { full_text: "", text_blocks: [] }, normalized_output: priorNormalizedOutput } },
  });
  const response = await handleBoundedEnrichStage({
    supabaseAdmin, orgId: ORG_ID, fileId: FILE_ID, pipelineJobId: JOB_ID, generationId: GEN_ID, stage: "enrich_clauses",
  ...
  assertEquals(supabaseAdmin.__updates.filter((u: any) => u.table === "uploaded_files").length, 0);
});
...
Deno.test("enrich-bounded-stages: enrich_fields resumes from a persisted enrich_clauses result without recomputing clauses", async () => {
  const clausesResult = runLeaseWorkflowStage1Clauses({ doclingRaw: { full_text: "Tenant shall pay rent.", text_blocks: [{ text: "Tenant shall pay rent.", page: 1 }] } });
  let normalizedOutput = mergeBoundedStageResult({
    normalizedOutput: { rows: [{}], method: "llm_only", warnings: [], validationErrors: [], metadata: {} },
    stage: "enrich_clauses", generationId: GEN_ID, status: "completed", data: clausesResult,
  });
  const supabaseAdmin = makeMockSupabase({
    rows: { uploaded_files: { id: FILE_ID, org_id: ORG_ID, active_generation_id: GEN_ID, module_type: "lease", docling_raw: { full_text: "Tenant shall pay rent.", text_blocks: [{ text: "Tenant shall pay rent.", page: 1 }] }, normalized_output: normalizedOutput } },
  });
  const response = await handleBoundedEnrichStage({
    supabaseAdmin, orgId: ORG_ID, fileId: FILE_ID, pipelineJobId: JOB_ID, generationId: GEN_ID, stage: "enrich_fields",
    jsonResponse: (body: any, status = 200) => new Response(JSON.stringify(body), { status }),
  });
  const body = await response.json();
  assertEquals(body.status, "completed");
  const results = readBoundedStageResults(supabaseAdmin.__rows.uploaded_files.normalized_output);
  assert(results.enrich_fields != null && results.enrich_fields.status === "completed");
...
Deno.test("enrich-bounded-stages: a terminal failure does NOT enqueue any further stage", async () => {
  const supabaseAdmin = makeMockSupabase({
    rows: { uploaded_files: { active_generation_id: GEN_ID, ui_review_payload: {} }, pipeline_jobs: { id: JOB_ID, status: "running" } },
  });
  const job = { id: JOB_ID, generation_id: GEN_ID, metadata: {} };
  await completeBoundedEnrichStage({
    supabaseAdmin, job, fileId: FILE_ID, orgId: ORG_ID, stage: "enrich_fields",
...
Deno.test("enrich-bounded-stages: pre-fence stale generation returns immediately, loads no data, persists nothing", async () => {
  const supabaseAdmin = makeMockSupabase({
    rows: { uploaded_files: { id: FILE_ID, org_id: ORG_ID, active_generation_id: "a-newer-generation" } },
  });
  const response = await handleBoundedEnrichStage({
    supabaseAdmin, orgId: ORG_ID, fileId: FILE_ID, pipelineJobId: JOB_ID, generationId: GEN_ID, stage: "enrich_clauses",
    jsonResponse: (body: any, status = 200) => new Response(JSON.stringify(body), { status }),
...
Deno.test("enrich-bounded-stages: completeBoundedEnrichStage detects a stale generation and cancels the job instead of persisting/enqueueing", async () => {
  const supabaseAdmin = makeMockSupabase({
    rows: { uploaded_files: { active_generation_id: "a-newer-generation" }, pipeline_jobs: { id: JOB_ID, status: "running" } },
  });
  const job = { id: JOB_ID, generation_id: GEN_ID, metadata: {} };
  const result = await completeBoundedEnrichStage({ supabaseAdmin, job, fileId: FILE_ID, orgId: ORG_ID, stage: "enrich_clauses", outcome: { ok: true } });
  ...
  assertEquals(supabaseAdmin.__rpcCalls.filter((c: any) => c.name === "enqueue_pipeline_job").length, 0);
});
...
Deno.test("enrich-bounded-stages: a generation change AFTER compute but BEFORE persist is caught by the post-fence check", async () => {
  ...
  const activeGenerationOverride = () => { callCount += 1; return callCount === 1 ? GEN_ID : "a-newer-generation"; };
  const supabaseAdmin = makeMockSupabase({
    rows: { uploaded_files: { id: FILE_ID, org_id: ORG_ID, active_generation_id: GEN_ID, module_type: "lease", docling_raw: { full_text: "", text_blocks: [] }, normalized_output: { rows: [{}], method: "llm_only", warnings: [], validationErrors: [], metadata: {} } } },
...
Deno.test("enrich-bounded-stages: enqueueBoundedEnrichStage's duplicate call returns the SAME existing job, not a second row", async () => {
  const supabaseAdmin = makeMockSupabase({
    rows: {},
    rpcResults: { enqueue_pipeline_job: { data: { created: false, existing_job_id: "existing-job-77" }, error: null } },
  });
  const result = await enqueueBoundedEnrichStage({
    supabaseAdmin, orgId: ORG_ID, fileId: FILE_ID, stage: "enrich_fields", generationId: GEN_ID,
    dispatchWorker: () => {},
  });
...
Deno.test("enrich-bounded-stages: an oversized stage input fails explicitly with BOUNDED_STAGE_LIMIT_EXCEEDED, never silently falls back to whole-document processing", async () => {
  ...
  const hugeTextBlocks = Array.from({ length: limits.maxTextBlocks + 1 }, (_, i) => ({ text: `block ${i}`, page: 1 }));
  const check = checkStageInputAgainstLimits("enrich_clauses", { textBlockCount: hugeTextBlocks.length, fullTextChars: 0, pageCount: 0 });
  assertEquals(check.withinLimits, false);

  const supabaseAdmin = makeMockSupabase({
    rows: { uploaded_files: { id: FILE_ID, org_id: ORG_ID, active_generation_id: GEN_ID, module_type: "lease", docling_raw: { full_text: "", text_blocks: hugeTextBlocks }, normalized_output: { rows: [{}], method: "llm_only", warnings: [], validationErrors: [], metadata: {} } } },
  });
  const response = await handleBoundedEnrichStage({
    supabaseAdmin, orgId: ORG_ID, fileId: FILE_ID, pipelineJobId: JOB_ID, generationId: GEN_ID, stage: "enrich_clauses",
    jsonResponse: (body: any, status = 200) => new Response(JSON.stringify(body), { status }),
...
Deno.test("enrich-bounded-stages: enrich_truth_assembly's own completion marks the chain complete and enqueues NOTHING further", async () => {
  ...
  const job = { id: JOB_ID, generation_id: GEN_ID, metadata: {} };
  const result = await completeBoundedEnrichStage({ supabaseAdmin, job, fileId: FILE_ID, orgId: ORG_ID, stage: "enrich_truth_assembly", outcome: { ok: true } });
...
const LEASE_DOCLING_RAW = { full_text: LEASE_TEXT, text_blocks: [{ text: LEASE_TEXT, page: 1 }], tables: [], fields: [] };
const LEASE_ROW = {
  tenant_name: "Cress Family Restaurants, LLC",
  landlord_name: "Markets at Choto, LLC",
  monthly_rent: 4500,
  _field_confidences: { tenant_name: 0.92, landlord_name: 0.92, monthly_rent: 0.9 },
};
const LEASE_RESULT = { rows: [LEASE_ROW], method: "llm_only", warnings: [], validationErrors: [], metadata: {} };
...
function buildPooledStandardFieldsForRow(): any[] {
  ...
  const evidenceByDomain = ["core_terms", "rent_and_charges", "expenses_and_cam", "operating_obligations", "legal_rights_and_dates"]
    .flatMap((domain) => buildStandardFieldsForEntries({ ...baseArgs, schemaEntries: getSchemaEntriesForDomain(allSchemaEntries, domain as any) }));
  const remainder = buildStandardFieldsForEntries({ ...baseArgs, schemaEntries: getSchemaEntriesWithNoDomain(allSchemaEntries) });
  ...
  const pooled = reorderStandardFieldsBySchema([...evidenceByDomain, ...remainder], allSchemaEntries);
...
Deno.test("enrich-bounded-stages: pooling all 4 workflow stages + all 5 evidence domains + remainder into buildReviewPayload produces output byte-equivalent to the monolithic call", () => {
  ...
  const boundedPayload = buildReviewPayload({
    fileId: FILE_ID, fileName: "lease.pdf", moduleType: "leases", documentSubtype: null,
    extractionMethod: "llm_only", reviewRequired: true, doclingRaw: LEASE_DOCLING_RAW, result: LEASE_RESULT,
  ...
  assertEquals({ ...boundedPayload, built_at: null }, { ...monolithicPayload, built_at: null });

  // 18. canonical payload contains truth-assembly markers.
  const tenantField = boundedPayload.records[0].standard_fields.find((f: any) => f.field_key === "tenant_name");
  assert(tenantField, "tenant_name must be present in the pooled standard_fields");
...
Deno.test("enrich-bounded-stages: pooled evidence covers fields from all 5 domains, not just one", () => {
  ...
  assert(keys.has("responsibility_taxes") || keys.has("tax_responsibility"), "expenses_and_cam field missing");
});
...
Deno.test("enrich-bounded-stages: a DOWNSTREAM_FUNCTION_FAILED (546-shaped) outcome is a plain terminal failure -- no soft/warning path exists in the bounded chain", async () => {
  const supabaseAdmin = makeMockSupabase({
    rows: { uploaded_files: { active_generation_id: GEN_ID, ui_review_payload: { records: [{ fields: {} }] } }, pipeline_jobs: { id: JOB_ID, status: "running" } },
  });
  const job = { id: JOB_ID, generation_id: GEN_ID, metadata: {} };
  const originalMessage = "Function failed due to not having enough compute resources (please check logs)";
  await completeBoundedEnrichStage({
    supabaseAdmin, job, fileId: FILE_ID, orgId: ORG_ID, stage: "enrich_derivation",
    outcome: { ok: false, errorCode: "DOWNSTREAM_FUNCTION_FAILED", errorMessage: originalMessage },
  });
  assertEquals(supabaseAdmin.__rows.pipeline_jobs.status, "failed");
  assertEquals(supabaseAdmin.__rows.pipeline_jobs.error_code, "DOWNSTREAM_FUNCTION_FAILED");
  // 17. original downstream error remains persisted verbatim.
  assertEquals(supabaseAdmin.__rows.pipeline_jobs.error_message, originalMessage);
  // 15. the payload is marked failed, never silently presented as ready.
  assertEquals(supabaseAdmin.__rows.uploaded_files.ui_review_payload.enrichment_status, "failed");
  assertNotEquals(supabaseAdmin.__rows.uploaded_files.ui_review_payload.enrichment_status, "completed_with_warnings");
});
