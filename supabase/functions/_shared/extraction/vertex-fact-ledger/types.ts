// @ts-nocheck
/**
 * Vertex Fact Ledger — Shared Types
 *
 * Types for the `BUSINESS_EXTRACTION_PROVIDER=vertex_fact_ledger` pipeline.
 ...
 *  vertex-fact-ledger module — built once per document by document-index.ts. */
...
export interface DocumentProfileClassification {
  ...
  confidence: number;
  /** "vertex" when the Vertex classifier call succeeded, "regex_fallback"
   *  when it fell back to lease-workflow.ts's detectDocumentProfileSignals(). */
  method: "vertex" | "regex_fallback";
...
export interface VertexFactLedgerInput {
...
export interface VertexFactLedgerOptions {
