...
import { expandSourceSnippetFromMatch } from "./source-snippets.ts";
...
const FIELD_SPECS = [
  { key: "lease_date", group: "lease_header", aliases: ["lease_date", "date_of_lease"], patterns: [/\b(?:lease date)\b[^\n]{0,30}?([A-Za-z]+\s+\d{1,2},?\s+\d{4}|\d{4}-\d{2}-\d{2}|\d{1,2}[\/-]\d{1,2}[\/-]\d{2,4})/i, /\bentered\s+into\s+as\s+of\s+([A-Za-z]+\s+\d{1,2},?\s+\d{4}|\d{4}-\d{2}-\d{2}|\d{1,2}(?:st|nd|rd|th)?\s+day\s+of\s+[A-Za-z]+,?\s+\d{4})/i, /\b(?:made\s+and\s+entered\s+into|entered\s+into)\s+(?:as\s+of\s+)?(?:this\s+)?(\d{1,2}(?:st|nd|rd|th)?\s+day\s+of\s+[A-Za-z]+,?\s+\d{4}|[A-Za-z]+\s+\d{1,2},?\s+\d{4})/i] },
...
function cleanSummaryValueForField(fieldKey: string, value: unknown): string | number | null {
  ...
  if (["tenant_name", "landlord_name", "broker_name"].includes(fieldKey)) {
    ...
    if (fieldKey === "broker_name" && /\b(?:costs?\s+of\s+reletting|reletting|damages?|repairs?|maintenance|alterations?|default|remedies?)\b/i.test(entity)) return null;
...
const UNIVERSAL_ITEM_DEFS = [
  { item_type: "lease_date", business_area: "dates_term", field_key: "lease_date", maps: true, patterns: [/\b(?:lease\s+date)\b[^\n]{0,40}?([A-Za-z]+\s+\d{1,2},?\s+\d{4})/i, /\b(?:made\s+and\s+entered\s+into|entered\s+into)\s+(?:as\s+of\s+)?(?:this\s+)?(\d{1,2}(?:st|nd|rd|th)?\s+day\s+of\s+[A-Za-z]+,?\s+\d{4}|[A-Za-z]+\s+\d{1,2},?\s+\d{4})/i] },
  ...
  if (
    ...
    if (derivedMonths) {
      fieldMap.lease_term_months = {
        ...
        value: derivedMonths,
        raw_value: derivedMonths,
        normalized_value: derivedMonths,
        source_page: fieldMap.expiration_date?.source_page ?? fieldMap.commencement_date?.source_page ?? null,
        source_clause: [fieldMap.commencement_date?.source_clause, fieldMap.expiration_date?.source_clause].filter(Boolean).join("\n") || null,
        exact_source_text: [fieldMap.commencement_date?.exact_source_text, fieldMap.expiration_date?.exact_source_text].filter(Boolean).join("\n") || null,
