// @ts-nocheck
/**
 * OpenAI Fact Ledger — Document Profile Classifier
 *
 * One callLLMJSON() call over the document's head/tail excerpt,
 * text-only (never file-mode — file-mode is reserved for fact-ledger-extractor.ts
 * and only under the explicit OPENAI_FACT_LEDGER_FILE_MODE opt-in flag).
 *
 ...
 * fallback on any OpenAI failure, malformed response, or invalid profile.
 */
