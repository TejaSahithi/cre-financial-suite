-- Readiness hardening: evaluate_cam_readiness could return ready=true while
-- a published, pool-assigned cam_expense_input still had variability=
-- 'unknown' -- CAM V2 then correctly blocked calculation itself
-- (VARIABILITY_UNKNOWN, engine-side, untouched here), but only after the
-- caller had already committed to running it. This closes that gap the
-- same way every other readiness condition in this function is closed:
-- one more read-only exception check, same shape, same severity model.
--
-- assign_cam_input_to_pool's pool-category default backfill (added in
-- 20269900000051) remains the normal, automatic way variability gets
-- resolved -- this check is the safety net for what that backfill can't
-- fix on its own: a pool/category with no variability_default configured
-- at all. It never guesses a value; it only reports.
--
-- Deliberately scoped to ASSIGNED inputs for pools in THIS recovery
-- period: an unassigned input already trips POOL_ASSIGNMENT_MISSING above
-- and never reaches the engine, so flagging its variability too would be a
-- duplicate, misleading blocker for something that isn't actually about to
-- run.
