-- Reliability Phase R1 (continued): teach the one SQL readiness authority
-- about the new enrichment_status = 'partial' terminal state added in
-- 20260881000000_enrichment_terminal_state_partial.sql.
--
-- Without this change, widening enrichment_status's CHECK constraint is
-- cosmetic: review_readiness is derived purely from pipeline_jobs.enrich
-- .status, and any 'failed' there is an unconditional ENRICHMENT_FAILED
-- reason that forces review_readiness = 'failed' -- still blocking the
-- reviewer from a document whose 88 core fields are already good. This
-- adds one new reason code, ENRICHMENT_PARTIAL, used instead of
-- ENRICHMENT_FAILED specifically when the enrich job failed AND
-- uploaded_files.enrichment_status = 'partial'. ENRICHMENT_PARTIAL is
-- deliberately left out of the hard-'failed' bucket in the readiness
-- derivation below, so it falls through to the existing 'partial'
-- catch-all -- review_readiness = 'partial' already exists and is already
-- documented for exactly this case ("some review data exists but
-- required work is incomplete -- reviewer may inspect for
-- troubleshooting"), so no new review_readiness value is needed.
--
-- Full function body redefined (matching every prior migration that has
-- touched this function), starting from the current body in
-- 20260865000000_openai_extraction_attempted_gate.sql.
