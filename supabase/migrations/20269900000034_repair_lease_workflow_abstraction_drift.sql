-- Root cause (confirmed against real production data, project
-- cjwdwuqqdokblakheyjb): 20260513133000_lease_workflow_abstraction.sql is
-- recorded as APPLIED in supabase_migrations.schema_migrations, but its
-- actual effect on production diverged from the file: public.leases is
-- missing all 29 columns that migration adds (commencement_date,
-- expiration_date, rentable_area_sqft, etc.), and public.cam_profiles does
-- not exist at all -- while public.lease_clauses (defined in the SAME
-- file) DOES exist. This is schema drift, not an intentional removal: no
-- later migration anywhere in this repo drops leases.commencement_date or
-- any of its 28 siblings (confirmed by search). The live symptom: every
-- RPC that reads leases.commencement_date --
-- backfill_cam_engine_v2_legacy_data among them -- fails with "column
-- l.commencement_date does not exist", which is exactly what blocked
-- "Prepare CAM Automatically" in production.
--
-- Fix: re-apply the original migration's content verbatim. Every
-- statement in it was already idempotent (ADD COLUMN IF NOT EXISTS,
-- CREATE TABLE IF NOT EXISTS, DROP POLICY/TRIGGER IF EXISTS before
-- CREATE), so doing this is safe regardless of which columns/objects are
-- actually present right now -- nothing here can duplicate an object or
-- overwrite existing data on rows that DO already have these columns
-- correctly (there are none, per the drift, but this makes the migration
-- correct even if remote state changes again before this runs).
