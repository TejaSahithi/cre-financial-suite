-- Reliability Phase R3: bounded enrichment readiness compatibility.
--
-- Bounded enrichment completes under enrich_truth_assembly instead of the
-- legacy monolithic enrich stage. Treat the latest enrich_truth_assembly row
-- as the enrichment terminal marker so Review Readiness does not report
-- ENRICHMENT_NOT_STARTED after bounded enrichment actually completed.
...
BEGIN
  ...
  -- Enrichment required by default for every lease document. The original
  -- monolithic path records stage = 'enrich'; the bounded path records its
  -- terminal publish step as stage = 'enrich_truth_assembly'. Either terminal
  -- marker is a valid enrichment completion signal for review readiness.
