-- UAT Finding C fix: evaluate_cam_readiness's POLICY_CONFLICT and
-- PREMISES_MISSING checks filtered only by org_id, never by p_property_id --
-- unlike every other check in this function (POLICY_MISSING, AREA_MISSING,
-- POOL_CATEGORY_MISSING, the unassigned/variability/allocation checks), all
-- of which correctly scope to p_property_id. Reproduced this UAT: running
-- readiness for a brand-new, fully unrelated QA staging property returned
-- ready=false with blocking exceptions naming leases on two different real
-- properties (Macon Crossing, 224 S Peters Road) -- no property in the org
-- could ever reach ready=true while any other lease anywhere had a policy
-- conflict or missing premises record.
--
-- Fix: join both checks through leases.property_id = p_property_id, the
-- exact pattern the POLICY_MISSING check above them already uses. Every
-- other check, and the rest of this function, is reproduced verbatim from
-- 20269900000052_cam_readiness_expense_variability_unresolved.sql. Signature
-- is unchanged (5 params), so a plain CREATE OR REPLACE is sufficient.
...
BEGIN
  ...
  -- Any lease_premises_spaces row claiming a building/unit that does not
  -- actually belong to this property+org.
  FOR v_row IN
  ...
  -- One row per (lease, approved rule) that has NO materialized policy at all.
  FOR v_row IN
  ...
  -- expense category with overlapping effective windows (a real
  -- POLICY_CONFLICT — the blueprint's own fail-closed conflict rule).
  -- Scoped to leases on THIS property: p1/p2 always share lease_id (see the
  -- p2.lease_id = p1.lease_id join below), so filtering p1's lease is
  -- sufficient to keep this from leaking conflicts belonging to unrelated
  -- properties into this property's readiness result.
  FOR v_row IN
  ...
  -- Scoped to leases on THIS property, same as POLICY_MISSING above.
  FOR v_row IN
  ...
  -- CAM V2 blocks gross-up on any assigned input still at variability=
  -- 'unknown' (VARIABILITY_UNKNOWN, engine-side, unchanged). The normal
  -- automatic resolution path is assign_cam_input_to_pool backfilling from
  -- recovery_pool_categories.variability_default; this only fires when
  -- that backfill had nothing to resolve from (no default configured for
  -- that pool/category), so setup fails closed here instead of the caller
  -- discovering it only after the engine already ran.
  FOR v_row IN
