-- Generic expense detail edits must not change workflow/review status.
-- Status transitions are owned by explicit review/approval commands; allowing
-- approval_status, approved_status, or review_status through this patch API
-- lets a browser-side detail edit bypass those commands.
...
DECLARE
  ...
  v_allowed_keys TEXT[] := ARRAY[
    ...
    'attachment_url', 'gl_code', 'invoice_number', 'source', 'source_type', 'source_file_id',    'fiscal_year', 'month',
...
BEGIN
  ...
  v_approval_status := COALESCE(v_before.approval_status, v_before.approved_status, 'approved');
  v_review_status := COALESCE(v_before.review_status, v_approval_status);
