-- Readiness/engine parity fix: a live end-to-end run-cam-calculation-v2
-- invocation against a fully "ready" QA fixture (per evaluate_cam_readiness)
-- still returned $0 instead of the expected amount, blocked by fine-grained
-- exceptions the pure engine (run-cam-engine.ts) raises that this SQL
-- readiness check never previewed:
--
--   * PREMISES_MISSING (poolScopedPremises, run-cam-engine.ts) -- the
--     existing PREMISES_MISSING check below only verifies a lease has *a*
--     lease_premises record somewhere; the engine additionally requires
--     that premises to be linked, via lease_premises_spaces, into the
--     PHYSICAL SCOPE of every pool the lease actively participates in.
--   * DENOMINATOR_ZERO (policy-step-runner.ts CALCULATE_SHARE step) -- for
--     an area-based (non fixed_percentage) recovery share, any calendar
--     month where the pool has no included published CAM dollars gets a
--     hard-coded zero denominator (run-cam-engine.ts's Pass-1 pool
--     assembly only computes a real denominator area for months where the
--     pool actually has money), which then blocks that month's share
--     calculation even though every other readiness check is green.
--   * AREA_MISSING for the POOL's own scope (denominator.ts's
--     computeDenominatorArea, entity_type space_area_measurements) -- for a
--     month the pool DOES have included dollars, the engine still requires
--     a rentable space_area_measurements row covering every one of the
--     pool's eligible scope ids (property/building/scope_members). The
--     existing AREA_MISSING check below only covers the TENANT's own
--     numerator area (lease_premises_area_periods); it never checked the
--     pool's denominator-side area at all.
--
-- All three conditions are added here as additional blocking checks,
-- reusing the exact codes/entity_types/messages the engine itself raises,
-- per the same "one authoritative readiness contract" principle the rest
-- of this function already follows -- not new parallel semantics.
--
-- Two small STABLE helper functions factor out scope/inclusion logic that
-- the new checks need (and that a future engine-parity fix would need
-- again), instead of duplicating it inline.

-- Mirrors run-cam-engine.ts's resolveEligibleScopeIds(pool, segment): an
-- active recovery_pool_scope_members override (include_in_denominator,
-- date-overlapping) wins; otherwise a property-scoped pool's eligible
-- scope is the property itself, a building-scoped pool's is its own
-- scope_id, and anything else (e.g. an unconfigured custom pool) is empty.
CREATE OR REPLACE FUNCTION public.cam_pool_eligible_scope_ids(p_pool_id UUID, p_from DATE, p_to DATE)
RETURNS UUID[]
LANGUAGE sql
STABLE
...
AS $$
  SELECT COALESCE(
    (SELECT array_agg(m.scope_id) FROM public.recovery_pool_scope_members m
      WHERE m.pool_id = p_pool_id AND m.include_in_denominator
        AND daterange(m.effective_from, m.effective_to, '[]') && daterange(p_from, p_to, '[]')),
    (SELECT CASE
              WHEN rp.scope_type = 'property' THEN ARRAY[rp.property_id]
              WHEN rp.scope_type = 'building' AND rp.scope_id IS NOT NULL THEN ARRAY[rp.scope_id]
              ELSE ARRAY[]::UUID[]
            END
       FROM public.recovery_pools rp WHERE rp.id = p_pool_id)
  );
$$;

REVOKE ALL ON FUNCTION public.cam_pool_eligible_scope_ids(UUID, DATE, DATE) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.cam_pool_eligible_scope_ids(UUID, DATE, DATE) TO service_role;

-- Mirrors pool-builder.ts's assembleSegmentAmounts (published + assigned +
-- service-period overlap) composed with applyCategoryInclusionExclusion
-- (an explicit 'exclude' entry always wins; otherwise included unless the
-- pool has 'include' entries and this category isn't one of them): does
-- this pool have at least one published CAM dollar, for this org/property,
-- that actually lands in the pool's recoverable total for this date range.
CREATE OR REPLACE FUNCTION public.cam_pool_has_included_amount(
  p_pool_id UUID, p_org_id UUID, p_property_id UUID, p_from DATE, p_to DATE
)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
...
AS $$
  SELECT EXISTS (
    SELECT 1
      ...
      JOIN public.cam_input_pool_assignments a ON a.cam_expense_input_id = cei.id AND a.recovery_pool_id = p_pool_id
     WHERE cei.org_id = p_org_id AND cei.property_id = p_property_id AND cei.publication_status = 'published'
       AND cei.service_period_start IS NOT NULL AND cei.service_period_end IS NOT NULL
       AND daterange(cei.service_period_start, cei.service_period_end, '[]') && daterange(p_from, p_to, '[]')
       AND (
         NOT EXISTS (SELECT 1 FROM public.recovery_pool_categories rpc WHERE rpc.pool_id = p_pool_id)
         OR (
           cei.expense_category_id IS NOT NULL
           AND NOT EXISTS (
             SELECT 1 FROM public.recovery_pool_categories rpc_ex
              WHERE rpc_ex.pool_id = p_pool_id AND rpc_ex.expense_category_id = cei.expense_category_id AND rpc_ex.inclusion_mode = 'exclude'
           )
           AND (
             EXISTS (
               SELECT 1 FROM public.recovery_pool_categories rpc_inc
                WHERE rpc_inc.pool_id = p_pool_id AND rpc_inc.expense_category_id = cei.expense_category_id AND rpc_inc.inclusion_mode = 'include'
             )
             OR NOT EXISTS (SELECT 1 FROM public.recovery_pool_categories rpc_any WHERE rpc_any.pool_id = p_pool_id AND rpc_any.inclusion_mode = 'include')
           )
         )
       )
  );
...
REVOKE ALL ON FUNCTION public.cam_pool_has_included_amount(UUID, UUID, UUID, DATE, DATE) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.cam_pool_has_included_amount(UUID, UUID, UUID, DATE, DATE) TO service_role;

-- Full evaluate_cam_readiness body, reproduced from 20269900000057 with two
-- new blocks inserted (pool-scoped PREMISES_MISSING right after the
-- existing coarse premises-existence check; DENOMINATOR_ZERO right after
-- POOL_ASSIGNMENT_MISSING) -- everything else is verbatim.
...
BEGIN
  ...
  -- Zero-denominator months: mirrors run-cam-engine.ts's pool assembly gate
  -- (a month's denominator area is only computed from real area data when
  -- the pool has at least one included published dollar that month; every
  -- other month silently carries denominatorArea = 0) composed with
  -- policy-step-runner.ts's CALCULATE_SHARE step, which raises a blocking
  -- DENOMINATOR_ZERO whenever ctx.denominatorArea <= 0 for an area-based
  -- (non fixed_percentage) share. A lease/pool/month combo where an
  -- area-based policy is active but the pool has no included dollars that
  -- month will hit this in the live engine even though every other
  -- readiness check above is green. Reuses the exact DENOMINATOR_ZERO code.
  --
  -- Approximated at calendar-month granularity -- the engine may further
  -- split a month at mid-month effective-date boundaries; this is a
  -- readiness preflight, not a re-implementation of the engine's exact
  ...
  FOR v_row IN
    SELECT DISTINCT rpo.id AS pool_id, rpo.name AS pool_name, rp.lease_id,
     ...
     WHERE rp.org_id = p_org_id AND rp.status = 'active'
       ...
       AND NOT public.cam_pool_has_included_amount(rpo.id, p_org_id, p_property_id, m.month_start::date, (m.month_start + INTERVAL '1 month - 1 day')::date)
  LOOP
    v_exceptions := v_exceptions || jsonb_build_object(
      'severity', 'blocking', 'code', 'DENOMINATOR_ZERO', 'entity_type', 'recovery_pools', 'entity_id', v_row.pool_id,
      'message', format('Pool %s ("%s") has no included published CAM dollars for %s..%s, so lease %s''s area-based recovery share will compute a zero denominator for that period', v_row.pool_id, COALESCE(v_row.pool_name, 'unnamed pool'), v_row.month_start, v_row.month_end, v_row.lease_id)
    );
  END LOOP;

  -- Pool-scope denominator area: mirrors denominator.ts's
  -- computeDenominatorArea, which the engine only invokes for a month the
  -- pool actually has included dollars (the DENOMINATOR_ZERO check above
  -- covers months where it doesn't), and which raises a blocking
  ...
  -- that month. Reuses the exact code/entity_type/severity.
  FOR v_row IN
