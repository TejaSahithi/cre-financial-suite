-- CAM Setup automation pass, item H: register the new BudgetReadiness page
-- in the backend page/module permission helpers, mirroring CAMSetup's
-- access level exactly (same roles, same 'write'/'read' outcome per role) --
-- consistent with 20260516170000_align_new_page_permissions.sql's own
-- stated purpose ("keep backend page/module permission helpers aligned with
-- the frontend nav"). Same function signatures as before, so a plain
...
RETURNS TEXT AS $$
  SELECT CASE
    ...
    WHEN page_name = ANY (ARRAY['CAMDashboard', 'CAMSetup', 'CAMCalculation', 'BudgetReadiness']) THEN 'cam'
  ...
  SELECT CASE
    ...
    WHEN role_key IN ('manager', 'asset_manager', 'portfolio_manager', 'operations_director', 'facility_manager', 'construction_manager', 'acquisitions_mgr', 'leasing_director')
      AND page_name = ANY (ARRAY[
        ...
        'CAMDashboard', 'CAMSetup', 'CAMCalculation', 'BudgetReadiness', 'Billing',
    ...
    WHEN role_key IN ('editor', 'financial_analyst', 'leasing_agent', 'lease_admin', 'finance', 'cfo_controller', 'accounts_manager')
      AND page_name = ANY (ARRAY[
        ...
        'Reconciliation', 'CAMDashboard', 'CAMSetup', 'CAMCalculation', 'BudgetReadiness', 'ChartOfAccounts',
    ...
    WHEN role_key IN ('viewer', 'read_only', 'investor_relations')
      AND page_name = ANY (ARRAY[
        ...
        'Comparison', 'AnalyticsReports', 'Reports', 'Analytics', 'CAMDashboard', 'CAMSetup', 'BudgetReadiness',
    ...
    WHEN role_key IN ('auditor', 'compliance_officer', 'internal_auditor')
      AND page_name = ANY (ARRAY[
        ...
        'AnalyticsReports', 'Reports', 'Analytics', 'CAMDashboard', 'CAMSetup', 'BudgetReadiness', 'Documents',
