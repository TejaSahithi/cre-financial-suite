-- Hardening Sprint 2 Phase 3: server-owned publish-to-CAM for lease rules.
-- Additive only: idempotent workflow runs and one transactional RPC.
...
BEGIN
  ...
  ELSE
    ...
    VALUES (
      ...
      'publish_rule_to_cam',
      'published_to_cam',
      COALESCE(v_before->>'published_to_cam', 'false'),
      'true',
      p_actor_user_id,
      p_actor_email,
      'info',
    ...
    VALUES (
      p_org_id,
      'approval',
      'Lease expense rule published to CAM',
      'An approved lease expense rule is now available for CAM setup.',
      '/LeaseExpenseClassification?lease=' || v_lease.id::TEXT,
      'normal'
    )
