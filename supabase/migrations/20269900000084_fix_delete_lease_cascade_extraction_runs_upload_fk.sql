-- Repair lease deletion when uploaded_files is still referenced by
-- extraction_runs through uploaded_file_id. The prior cascade collected those
-- run IDs but only reliably deleted extraction_runs rows with lease_id set;
-- draft/review uploads can leave extraction_runs.lease_id NULL, so deleting
-- uploaded_files failed with extraction_runs_uploaded_file_id_org_id_fkey.
-- Keep the function schema-tolerant: tables and columns can vary by deployment.
