-- ===========================================================================
-- Enterprise CAM & Budget Implementation Blueprint v1.0 — Phase 2C:
-- readiness engine.
--
-- evaluate_cam_readiness(org_id, property_id, recovery_period_id,
-- scope_type, scope_id) returns a machine-readable JSONB array of
-- exceptions — each {severity, code, entity_type, entity_id, message} —
-- for the readiness conditions the blueprint lists. It never writes
-- anything (read-only evaluation; Phase 2 has no cam_run to attach real
-- cam_run_exceptions rows to yet — a future create-cam-run workflow would
-- call this and persist the results as cam_run_exceptions). Codes follow
-- the blueprint's own Appendix B catalog where one exists; two new codes
-- were needed for conditions the catalog doesn't name:
--   RECOVERY_PERIOD_NOT_READY  — the period has no pools configured yet
--   CROSS_ORG_REFERENCE        — a scope_id resolves outside the caller's org
--
-- "Overlapping premises/area/occupancy/policy periods" is deliberately NOT
-- re-checked here: the EXCLUDE constraints added in the hardened Phase 1
-- migrations (20269900000008-3) make that state impossible to create in
-- the first place, so there is nothing for a readiness scan to find.
-- ===========================================================================
