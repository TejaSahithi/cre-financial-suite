-- ============================================================
-- APPLY ALL MISSING v2 — fixed ordering
-- Functions are created FIRST, then tables that depend on them.
-- Run this in Supabase SQL Editor.
-- ============================================================

-- ── STEP 1: Core helper functions (must exist before any RLS policy) ──────

-- can_write_org_data: super_admins can write to any org
...
-- fn_set_updated_at: generic trigger function
...
-- ── STEP 2: Rebuild RLS on existing tables with super_admin bypass ────────
DO $$
DECLARE t TEXT;
BEGIN
...
-- Back-fill progress for existing rows
UPDATE public.uploaded_files SET progress_percentage =
  CASE status
    WHEN 'uploaded'   THEN 5   WHEN 'parsing'    THEN 15
    WHEN 'parsed'     THEN 30  WHEN 'pdf_parsed' THEN 35
    WHEN 'validating' THEN 45  WHEN 'validated'  THEN 60
    WHEN 'storing'    THEN 70  WHEN 'stored'     THEN 80
    WHEN 'computing'  THEN 90  WHEN 'completed'  THEN 100
    WHEN 'processed'  THEN 100 ELSE 0
  END
...
-- ── STEP 4: computation_snapshots table ───────────────────────────────────
...
-- ── STEP 6: Extend portfolios, buildings, units ───────────────────────────
...
-- ── STEP 8: Access helper functions ──────────────────────────────────────
